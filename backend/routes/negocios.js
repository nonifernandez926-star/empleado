...
router.put('/mi-negocio', requiereAdmin, async (req, res) => {
  try {
    ...
    if (Array.isArray(zonasDelivery)) {
      req.negocio.zonasDelivery = zonasDelivery
        .filter((z) => z && z.zona && String(z.zona).trim())
        .map((z) => ({ zona: String(z.zona).trim(), precio: Math.max(0, Number(z.precio) || 0) }));
    }
...
router.post('/promociones', requiereAdmin, async (req, res) => {
  try {
    const { titulo, descripcion, fechaHasta, horarioDesde, horarioHasta, aplicaA, usosMaximos } = req.body;
    if (!titulo || !titulo.trim()) return res.status(400).json({ error: 'El título es obligatorio' });

    req.negocio.promociones.push({
      titulo: titulo.trim(),
      descripcion: (descripcion || '').trim(),
      activa: true,
      fechaHasta: fechaHasta || undefined,
      horarioDesde: horarioDesde || undefined,
      horarioHasta: horarioHasta || undefined,
      aplicaA: (aplicaA || '').trim(),
      usosMaximos: usosMaximos !== undefined && usosMaximos !== '' ? Number(usosMaximos) : undefined,
    });
    await req.negocio.save();
...
router.put('/promociones/:id', requiereAdmin, async (req, res) => {
  try {
    ...
    const { titulo, descripcion, activa, fechaHasta, horarioDesde, horarioHasta, aplicaA, usosMaximos, usosActuales } = req.body;
    ...
    if (fechaHasta !== undefined) promo.fechaHasta = fechaHasta || undefined;
    if (horarioDesde !== undefined) promo.horarioDesde = horarioDesde || undefined;
    if (horarioHasta !== undefined) promo.horarioHasta = horarioHasta || undefined;
    if (aplicaA !== undefined) promo.aplicaA = aplicaA;
    if (usosMaximos !== undefined) promo.usosMaximos = usosMaximos === '' ? undefined : Number(usosMaximos);
    if (usosActuales !== undefined) promo.usosActuales = Number(usosActuales);
