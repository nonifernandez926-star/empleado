...
// avisarle al cliente si esta en el top 3) y el texto del premio para cada puesto
router.put('/config', requiereAdmin, async (req, res) => {
  try {
    ...
    if (premios) {
      req.negocio.ranking.premios = {
        top1: String(premios.top1 || '').trim(),
        top2: String(premios.top2 || '').trim(),
        top3: String(premios.top3 || '').trim(),
      };
    }
